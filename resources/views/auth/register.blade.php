        ...
        <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md relative z-10">
            <div class="bg-slate-900 py-8 px-4 shadow-2xl border border-slate-800 rounded-2xl sm:px-10 backdrop-blur-sm">
                <form class="space-y-5" action="{{ route('register') }}" method="POST">
                    ...
                    <x-form-field label="Name" type="text" name="name" placeholder="Enter full name*" required />
                    <x-form-field label="Email" type="email" name="email" placeholder="Enter email id*" required />
                    <x-form-field label="Password" type="password" name="password" required />
                    <x-form-field label="Confirm Password" type="password" name="password_confirmation" required />

                    <div class="flex items-center">
                        <input id="terms" name="terms" type="checkbox" class="h-4 w-4 rounded border-slate-700 bg-slate-950 text-emerald-500 focus:ring-emerald-500/50" required>
                        <label for="terms" class="ml-2 block text-sm text-slate-400">
                            I agree to the <a href="#" class="text-emerald-400 hover:text-emerald-300 underline underline-offset-4">Terms of Service</a>
                        </label>
                    </div>
